...
public class loginViewClass extends AppCompatActivity implements loginViewInterface {
    ...
    public void setNavigation() {
       ...
       /* List<Person_det>stList=Methods.getDefaultStudent(getApplicationContext());
        ...
        Log.e("stList","stList "+stList.size());
        person_det=stList.get(0);

        StudenId=person_det.getStudentId();
        String studentname=person_det.getFirstName();

        SharedPreferences.Editor editorstudent = studentPreference.edit();
        editorstudent.putString("studentid", StudenId);
        editorstudent.putString("studentname",studentname);
        editorstudent.commit();
*/

        Log.e("delte navigation","delte navigation");
