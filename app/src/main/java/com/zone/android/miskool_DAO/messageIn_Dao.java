...
public interface  messageIn_Dao {
    ...
    /*@Query("SELECT * FROM Message_In where message_sender= :student_id")
    List<Message_In> getInboxMessagesStudent(String student_id);
*/
   /* @Query("SELECT message_date,message_sub,thread_id ,messages,message_sender,message_receiver FROM Message_In where message_receiver= :student_id " +
            "group by thread_id order by message_date desc")
    List<Message_In> getInboxMessagesStudent(String student_id);
*/
...
*/
   ...
   //the original inbox added by thread

    /*@Query("select a.message_date, a.message_sub,a.messages from (select * from (select * from Message_In where message_receiver= :student_id order by message_date asc)" +
            " As b   group by b.thread_id ) " +
            " As a order by a.message_date desc")
    List<Message_In> getInboxMessagesStudent(String student_id);
*/
   ...
   /* @Query("SELECT * FROM Message_In where message_receiver= :student_id and thread_id= :Thread_id " +
            "order by message_date desc")
    List<Message_In> getInboxMessagesStudentThread(String student_id, String Thread_id);*/




   /* @Query("SELECT * FROM Message_In where message_receiver= :student_id and messagein_id= :messagein_id " +
            "order by message_dateCreated desc")
    List<Message_In> getInboxMessagesStudentThread(String student_id, String messagein_id);*/
   ...
   // @Query("UPDATE Message_In SET read_flag = :1  WHERE thread_id = :thread_id)
